...
class StampProgramCreate(BaseModel):
    ...
    accent_color: str = Field(default="#C6FF4A", pattern=r"^#[0-9A-Fa-f]{6}$")
    display_options: dict = Field(default_factory=lambda: {
        "icon_size": 42, "gap": 10, "offset_x": 0, "offset_y": 0,
        "fit": "contain", "shape": "none", "empty_opacity": 35,
    })
...
class CustomerCardAssignmentsSet(BaseModel):
    card_template_ids: list[UUID] = Field(default_factory=list, max_length=30)
