...
import {
  Activity, Bell, Building2, Camera, CheckCircle2, Coffee, Copy, CreditCard, Gift, ImageIcon, Loader2,
  MapPin, Pencil, Plus, QrCode, RefreshCw, Save, ScanLine, Send, ShieldCheck, Sparkles, Stamp,
  ...
  const loadTab=useCallback(async()=>{
    ...
    try{
      ...
      if(tab==='customers'){
        ...
        const [rewardsResult,couponsResult,branchesResult]=await Promise.allSettled([
        ...
        payload={customers,rewards:safeArray(settledValue(rewardsResult,[])),coupons:safeArray(settledValue(couponsResult,[])),branches:safeArray(settledValue(branchesResult,[]))}
      }
      if(tab==='staff'){
      ...
      if(tab==='wallet'){
        const [design,customers,programs]=await Promise.all([
        ...
        payload={design,customers:safeArray(customers),programs:safeArray(programs)}
      }
  ...
  return <Shell active={tab} onChange={setTab} role={me.role} accessRole={selectedAccess?.role} userName={me.name} brandName={selected?.name} capabilities={selectedCaps} permissions={selectedPermissions}>
    <div className="admin-container">
      ...
      <SectionErrorBoundary key={`${brand}-${tab}`} onRetry={loadTab}>
        ...
        {tab==='customers'&&<Customers rows={safeArray(data.customers)} rewards={safeArray(data.rewards)} coupons={safeArray(data.coupons)} branches={safeArray(data.branches)} brand={brand} submit={submitJson} busy={busy} loyaltyAction={loyaltyAction} issuePass={issuePass} run={run} reload={loadTab} capabilities={selectedCaps} permissions={selectedPermissions} setTab={setTab}/>} 
        ...
        {tab==='scan'&&<FastScan brand={brand} branches={safeArray(data.branches)} busy={busy} run={run}/>} 
        ...
        {tab==='wallet'&&<WalletStudio design={data.design||emptyDesign} customers={safeArray(data.customers)} programs={safeArray(data.programs)} brand={brand} brandInfo={selected} busy={busy} run={run} reload={loadTab} issuePass={issuePass}/>} 
...
const titles:Row={overview:'مركز التحكم',brands:'إدارة البراندات',branches:'الفروع',customers:'العملاء',staff:'الموظفون والصلاحيات','stamp-cards':'بطاقات الأختام',scan:'السكان السريع',loyalty:'محرك الولاء',wallet:'استوديو Apple Wallet',campaigns:'الإشعارات والحملات',audit:'سجل التدقيق','platform-wallet':'شهادة Apple Wallet المركزية'}
const subtitles:Row={overview:'متابعة المنصة والعمليات من مكان واحد.',brands:'أنشئ البراند وحدد نوع البرنامج والمميزات لكل حساب.',branches:'إدارة الفروع وبياناتها التشغيلية.',customers:'تسجيل العملاء وتحديث الولاء وإصدار البطاقات.',staff:'حساب مستقل لكل مدير أو موظف وصلاحيات معزولة.','stamp-cards':'أنشئ بطاقات قهوة وسويت وغيرها؛ لكل بطاقة تصميم ومكافأة مستقلة.',scan:'امسح رمز العميل واختر البطاقة ثم أضف الختم مباشرة.',loyalty:'قواعد النقاط والمستويات والمكافآت حسب نوع البراند.',wallet:'مدير البراند يصمم؛ مدير المنصة يحتفظ بالشهادة.',campaigns:'حملات فورية أو مجدولة مع حالة إرسال حقيقية.',audit:'كل تعديل مهم محفوظ وقابل للمراجعة.','platform-wallet':'لا يظهر هذا القسم إلا لمدير المنصة.'}

function Topbar({title,subtitle,brands,brand,setBrand,showBrand,reload}:any){const rows=safeArray<Row>(brands);return <header className="topbar"><div><p className="eyebrow">LOYALYN CONTROL CENTER · V4.1</p><h1>{title}</h1><p className="muted mt-2">{subtitle}</p></div><div className="top-actions">{showBrand&&<select className="input brand-select" value={brand} onChange={e=>setBrand(e.target.value)} aria-label="اختيار البراند">{rows.map((x:Row)=><option key={x.id} value={x.id}>{x.name}</option>)}</select>}<button type="button" className="icon-btn" onClick={reload} title="تحديث" aria-label="تحديث القسم"><RefreshCw size={18}/></button></div></header>}
...
function StampCards({rows,brand,brandInfo,busy,run,reload}:any){
  const [editing,setEditing]=useState<Row|null>(null)
  async function create(e:React.FormEvent<HTMLFormElement>){e.preventDefault();const form=e.currentTarget,fd=new FormData(form);await run('stamp-program-create',()=>api('/api/stamps/programs',{method:'POST',body:JSON.stringify({brand_id:brand,name:fd.get('name'),slug:String(fd.get('slug')||'').trim().toLowerCase(),description:fd.get('description')||null,required_stamps:Number(fd.get('required_stamps')||10),reward_title:fd.get('reward_title'),reward_type:fd.get('reward_type'),stamp_icon:fd.get('stamp_icon'),background_color:fd.get('background_color'),accent_color:fd.get('accent_color'),is_default:fd.get('is_default')==='on',sort_order:Number(fd.get('sort_order')||0)})}),'تم إنشاء بطاقة الأختام');form.reset();await reload()}
  async function update(e:React.FormEvent<HTMLFormElement>){e.preventDefault();if(!editing)return;const fd=new FormData(e.currentTarget);await run(`stamp-edit-${editing.id}`,()=>api(`/api/stamps/programs/${editing.id}`,{method:'PATCH',body:JSON.stringify({name:fd.get('name'),slug:String(fd.get('slug')||'').trim().toLowerCase(),description:fd.get('description')||null,required_stamps:Number(fd.get('required_stamps')),reward_title:fd.get('reward_title'),reward_type:fd.get('reward_type'),stamp_icon:fd.get('stamp_icon'),background_color:fd.get('background_color'),accent_color:fd.get('accent_color'),is_default:fd.get('is_default')==='on',sort_order:Number(fd.get('sort_order')||0),is_active:fd.get('is_active')==='on'})}),'تم تحديث بطاقة الأختام');setEditing(null);await reload()}
  ...
  return <><div className="content-grid mt-6"><Panel title="إنشاء بطاقة أختام" desc="كل بطاقة مستقلة: قهوة، سويت، فطور أو أي منتج"><form className="form-grid" onSubmit={create}><div className="two-col"><Field label="اسم البطاقة" name="name" placeholder="قهوة"/><Field label="الرابط الداخلي" name="slug" placeholder="coffee" pattern="[a-z0-9-]+"/></div><Field label="الوصف" name="description" required={false}/><div className="two-col"><Field label="عدد الأختام" name="required_stamps" type="number" defaultValue="6"/><Field label="اسم المكافأة" name="reward_title" defaultValue="مشروب مجاني"/></div><div className="two-col"><Field label="نوع المكافأة"><select className="input" name="reward_type"><option value="free_item">منتج مجاني</option><option value="discount">خصم</option><option value="custom">مخصص</option></select></Field><Field label="رمز البطاقة"><select className="input" name="stamp_icon"><option value="coffee">قهوة</option><option value="cake">سويت</option><option value="cookie">كوكيز</option><option value="cup">مشروب</option><option value="star">نجمة</option></select></Field></div><div className="two-col"><Field label="لون البطاقة" name="background_color" type="color" defaultValue={brandInfo?.primary_color||'#111827'}/><Field label="لون الأختام" name="accent_color" type="color" defaultValue={brandInfo?.accent_color||'#C6FF4A'}/></div><Field label="الترتيب" name="sort_order" type="number" defaultValue="0"/><Switch label="البطاقة الافتراضية" name="is_default"/><Btn busy={busy==='stamp-program-create'} label="إنشاء البطاقة" icon={Plus}/></form></Panel><Panel title="QR التسجيل الثابت" desc="اطبعه عند الكاشير أو على الطاولات؛ العميل يسجل ويضيف البطاقة"><div className="join-qr-card"><img src={`${API}/api/public/brands/${brandInfo?.slug}/join-qr.svg`} alt="QR التسجيل"/><div><b>{brandInfo?.name}</b><p>رابط ثابت لا يتغير</p><code>{joinUrl}</code><div className="button-row mt-3"><Btn label="نسخ الرابط" icon={Copy} variant="secondary" onClick={async()=>{await navigator.clipboard.writeText(joinUrl)}}/><a className="btn secondary" href={`${API}/api/public/brands/${brandInfo?.slug}/join-qr.svg`} target="_blank"><QrCode size={18}/>فتح QR</a></div></div></div></Panel></div><Panel title="البطاقات الحالية" desc="ارفع تصميم البطاقة وصورة الختم الفارغ والممتلئ" className="mt-5">{rows.length?<div className="stamp-program-grid">{rows.map((x:Row)=><div className={`stamp-program-card ${!x.is_active?'disabled':''}`} key={x.id} style={{backgroundColor:x.background_color,color:'#fff',backgroundImage:x.card_image_url?`linear-gradient(rgba(0,0,0,.25),rgba(0,0,0,.45)),url(${API}/api/stamps/public/assets/${x.id}/card)`:undefined}}><div className="stamp-program-head"><span>{stampIcon(x.stamp_icon)}</span><div><b>{x.name}</b><small>{x.required_stamps} أختام · {x.reward_title}</small></div>{x.is_default&&<em>افتراضية</em>}</div><StampDots count={Math.min(4,x.required_stamps)} total={x.required_stamps} program={x}/><div className="stamp-assets"><label><ImageIcon size={16}/>تصميم البطاقة<input type="file" accept="image/*" onChange={e=>asset(x,'card',e.target.files?.[0])}/></label><label><Upload size={16}/>الختم الفارغ<input type="file" accept="image/*" onChange={e=>asset(x,'empty_stamp',e.target.files?.[0])}/></label><label><Upload size={16}/>الختم الممتلئ<input type="file" accept="image/*" onChange={e=>asset(x,'filled_stamp',e.target.files?.[0])}/></label></div><Btn label="تعديل البطاقة" icon={Pencil} variant="secondary" onClick={()=>setEditing(x)}/></div>)}</div>:<Empty text="أنشئ أول بطاقة أختام"/>}</Panel>{editing&&<Modal title="تعديل بطاقة الأختام" onClose={()=>setEditing(null)}><form className="form-grid" onSubmit={update}><div className="two-col"><Field label="الاسم" name="name" defaultValue={editing.name}/><Field label="الرابط" name="slug" defaultValue={editing.slug}/></div><Field label="الوصف" name="description" defaultValue={editing.description||''} required={false}/><div className="two-col"><Field label="عدد الأختام" name="required_stamps" type="number" defaultValue={editing.required_stamps}/><Field label="المكافأة" name="reward_title" defaultValue={editing.reward_title}/></div><div className="two-col"><Field label="نوع المكافأة"><select className="input" name="reward_type" defaultValue={editing.reward_type}><option value="free_item">منتج مجاني</option><option value="discount">خصم</option><option value="custom">مخصص</option></select></Field><Field label="الرمز"><select className="input" name="stamp_icon" defaultValue={editing.stamp_icon}><option value="coffee">قهوة</option><option value="cake">سويت</option><option value="cookie">كوكيز</option><option value="cup">مشروب</option><option value="star">نجمة</option></select></Field></div><div className="two-col"><Field label="لون البطاقة" name="background_color" type="color" defaultValue={editing.background_color}/><Field label="لون الأختام" name="accent_color" type="color" defaultValue={editing.accent_color}/></div><Field label="الترتيب" name="sort_order" type="number" defaultValue={editing.sort_order}/><Switch label="البطاقة الافتراضية" name="is_default" defaultChecked={editing.is_default}/><Switch label="البطاقة فعالة" name="is_active" defaultChecked={editing.is_active}/><Btn busy={busy===`stamp-edit-${editing.id}`} label="حفظ التعديلات"/></form></Modal>}</>
}

function FastScan({brand,branches,busy,run}:any){
  ...
  useEffect(()=>{if(branchRows.length===1)setBranch(branchRows[0].id);else if(branch&&!branchRows.some(x=>x.id===branch))setBranch('')},[branches])
  async function lookup(value=code){const clean=value.trim();if(!clean)return;const valueData=await run('scan-lookup',()=>api(`/api/stamps/scan/${encodeURIComponent(clean)}?brand_id=${brand}`));if(valueData){setResult(valueData);setCode(clean)}}
  async function add(program:Row){if(!result)return;const valueData=await run(`scan-add-${program.id}`,()=>api(`/api/stamps/customers/${result.customer.id}/programs/${program.id}/add`,{method:'POST',body:JSON.stringify({branch_id:branch||null,quantity:1,idempotency_key:crypto.randomUUID()})}),`تمت إضافة ختم إلى ${program.name}`);if(valueData)setResult({...result,cards:safeArray(valueData.cards)})}
  async function redeem(program:Row){if(!result)return;const valueData=await run(`scan-redeem-${program.id}`,()=>api(`/api/stamps/customers/${result.customer.id}/programs/${program.id}/redeem`,{method:'POST',body:JSON.stringify({branch_id:branch||null,idempotency_key:crypto.randomUUID()})}),`تم صرف ${program.reward_title}`);if(valueData)setResult({...result,cards:safeArray(valueData.cards)})}
    ...
    try{
      ...
      window.setTimeout(async()=>{
        ...
        const tick=async()=>{
          ...
          try{const codes=await detector.detect(videoRef.current);if(codes?.[0]?.rawValue){setCode(codes[0].rawValue);await stopCamera();await lookup(codes[0].rawValue);return}}catch{}
  ...
  useEffect(()=>{
    const pending=localStorage.getItem('loyalyn_scan_code')
    if(pending){localStorage.removeItem('loyalyn_scan_code');setCode(pending);window.setTimeout(()=>void lookup(pending),120)}
    return ()=>{streamRef.current?.getTracks().forEach(t=>t.stop())}
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[brand])
  const cards=safeArray<Row>(result?.cards).filter(x=>x.card_active&&x.is_active)
  return <div className="scan-layout mt-6">
    <Panel title="مسح بطاقة العميل" desc="امسح QR أو اكتب رقم العضوية؛ بعدها اختر القهوة أو السويت وأضف الختم">
      <div className="scan-controls">
        <Field label="رقم العضوية أو نتيجة المسح"><input className="input scan-input" value={code} onChange={e=>setCode(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')void lookup()}} autoFocus placeholder="امسح الرمز هنا" inputMode="text"/></Field>
        ...
        {cameraError&&<div className="inline-error"><XCircle size={18}/><span>{cameraError}</span></div>}
        {camera&&<video ref={videoRef} className="scan-video" muted playsInline/>}
      </div>
    </Panel>
    <div className="scan-result">{result?<>
      <div className="scan-customer"><div className="avatar large">{result.customer?.name?.[0]||'ع'}</div><div><small>العميل</small><h2>{result.customer?.name}</h2><p>{result.customer?.phone} · {result.customer?.membership_code}</p></div><span className="badge success">جاهز</span></div>
      <div className="scan-card-grid">{cards.map((x:Row)=><div className="scan-stamp-card" key={x.id} style={{backgroundColor:x.background_color,backgroundImage:x.card_image_url?`linear-gradient(rgba(0,0,0,.28),rgba(0,0,0,.5)),url(${API}/api/stamps/public/assets/${x.id}/card)`:undefined}}><div className="stamp-program-head"><span>{stampIcon(x.stamp_icon)}</span><div><b>{x.name}</b><small>{x.reward_title}</small></div></div><StampDots count={x.stamps} total={x.required_stamps} program={x}/><div className="scan-card-meta"><span><b>{x.stamps}</b>/{x.required_stamps} ختم</span><span><b>{x.rewards_available}</b> مكافأة جاهزة</span></div><div className="button-row"><Btn busy={busy===`scan-add-${x.id}`} label="+ ختم" icon={Plus} onClick={()=>void add(x)}/><Btn busy={busy===`scan-redeem-${x.id}`} disabled={!x.rewards_available} label="صرف المكافأة" icon={Gift} variant="secondary" onClick={()=>void redeem(x)}/></div></div>)}</div>
      {!cards.length&&<Empty text="لا توجد بطاقات أختام فعالة لهذا العميل"/>}
    </>:<div className="scan-empty"><QrCode/><h3>بانتظار مسح العميل</h3><p>بعد المسح تظهر كل بطاقات الأختام المحفوظة وتختار البطاقة المناسبة مباشرة.</p></div>}</div>
  </div>
}
...
function Customers({rows,rewards,coupons,branches,brand,submit,busy,loyaltyAction,issuePass,run,reload,capabilities,permissions,setTab}:any){
  ...
  const canReverse=perms['*']===true||perms['loyalty.manage']===true
  ...
  return <div className="wide-grid mt-6">
    {canCreate&&<Panel title="تسجيل عميل" desc={caps.stamps?'بعد التسجيل ينضم العميل تلقائيًا لبطاقات الأختام الفعالة ويمكنه استخدام QR مباشرة.':'يمكن ربط العميل بفرعه المعتاد واستخدامه في برنامج البراند.'}>
      <form className="form-grid" onSubmit={(e)=>submit('/api/customers',e,{brand_id:brand},'POST','تم تسجيل العميل')}>
        ...
        {branchRows.length===1&&<input type="hidden" name="home_branch_id" value={branchRows[0].id}/>}<Field label="ملاحظات" name="notes" required={false}/><Btn busy={busy==='/api/customers'} label="إضافة العميل" icon={Plus}/>
      </form>
    </Panel>}
...
function Staff({rows,branches,brand,busy,run,reload,permissions}:any){
  ...
  const managerPreset:Record<string,boolean>={'brand.view':true,'branches.view':true,'customers.view':true,'customers.list':true,'customers.create':true,'customers.edit':true,'customers.history':true,'loyalty.view':true,'loyalty.apply':true,'loyalty.manual':true,'rewards.redeem':true,'wallet.view':true,'wallet.issue':true,'fast_scan.use':true,'campaigns.view':true}
  const permissionChoices=[
    ['fast_scan.use','السكان السريع'],['customers.view','البحث وعرض العميل'],['customers.list','عرض قائمة العملاء كاملة'],['customers.create','تسجيل عميل جديد'],['customers.edit','تعديل بيانات العملاء'],['customers.history','عرض سجل عمليات العميل'],['loyalty.apply','إضافة زيارة أو عملية ولاء'],['loyalty.manual','تعديل الرصيد يدويًا'],['rewards.redeem','صرف المكافآت'],['wallet.issue','إصدار بطاقة Wallet'],
...
function WalletStudio({design,customers,programs,brand,brandInfo,busy,run,reload,issuePass}:any){
  ...
  async function save(){await run('design-save',()=>api(`/api/wallet/design/${brand}`,{method:'PUT',body:JSON.stringify(d)}),'تم حفظ مسودة التصميم');reload()}
  async function publish(){await run('design-publish',()=>api(`/api/wallet/design/${brand}/publish`,{method:'POST'}),'تم نشر التصميم وتحديد البطاقات للتحديث');reload()}
  async function asset(e:React.ChangeEvent<HTMLInputElement>,kind:'logo'|'hero'|'background'|'strip'){const file=e.target.files?.[0];if(!file)return;const fd=new FormData();fd.append('kind',kind);fd.append('file',file);await run(`asset-${kind}`,()=>api(`/api/wallet/design/${brand}/asset`,{method:'POST',body:fd}),'تم رفع صورة التصميم');e.target.value='';reload()}
  ...
  return <div className="studio-grid mt-6"><Panel title="إعدادات التصميم" desc="ارفع خلفية كاملة أو صورًا منفصلة؛ الشهادة تبقى عند مدير المنصة"><div className="form-grid"><div className="two-col"><ColorInput label="الخلفية" value={d.background_color} onChange={(v:string)=>setD({...d,background_color:v})}/><ColorInput label="النص" value={d.foreground_color} onChange={(v:string)=>setD({...d,foreground_color:v})}/></div><ColorInput label="العناوين" value={d.label_color} onChange={(v:string)=>setD({...d,label_color:v})}/><div className="two-col"><Field label="اسم الشعار"><input className="input" value={d.logo_text} onChange={e=>setD({...d,logo_text:e.target.value})}/></Field><Field label="عنوان البطاقة"><input className="input" value={d.card_title} onChange={e=>setD({...d,card_title:e.target.value})}/></Field></div><div className="two-col"><Field label="نمط التصميم"><select className="input" value={d.layout_style} onChange={e=>setD({...d,layout_style:e.target.value})}><option value="classic">كلاسيكي</option><option value="visual">صورة كاملة</option><option value="minimal">بسيط</option></select></Field><NumberInput label="تعتيم الخلفية %" value={d.overlay_opacity} min={0} onChange={(v:number)=>setD({...d,overlay_opacity:Math.min(90,v)})}/></div><div className="asset-grid four"><AssetUpload label="رفع الشعار" busy={busy==='asset-logo'} onChange={(e:any)=>asset(e,'logo')}/><AssetUpload label="الصورة العلوية" busy={busy==='asset-hero'} onChange={(e:any)=>asset(e,'hero')}/><AssetUpload label="خلفية كاملة" busy={busy==='asset-background'} onChange={(e:any)=>asset(e,'background')}/><AssetUpload label="شريط Apple" busy={busy==='asset-strip'} onChange={(e:any)=>asset(e,'strip')}/></div><Field label="نوع الباركود"><select className="input" value={d.barcode_format} onChange={e=>setD({...d,barcode_format:e.target.value})}><option value="PKBarcodeFormatQR">QR</option><option value="PKBarcodeFormatPDF417">PDF417</option><option value="PKBarcodeFormatAztec">Aztec</option><option value="PKBarcodeFormatCode128">Code 128</option></select></Field><Field label="الشروط والأحكام" required={false}><textarea className="input min-h-24" value={d.terms||''} onChange={e=>setD({...d,terms:e.target.value})}/></Field><div className="switch-grid">{fieldOptions.filter((x:any)=>x[2]).map(([key,label]:any)=><label className="switch-row" key={key}><input type="checkbox" checked={!!d.fields[key]} onChange={e=>setD({...d,fields:{...d.fields,[key]:e.target.checked}})}/><span>{label}</span></label>)}</div><div className="button-row"><Btn busy={busy==='design-save'} label="حفظ المسودة" onClick={save}/><Btn busy={busy==='design-publish'} label="نشر التصميم" icon={Upload} variant="secondary" onClick={publish}/></div><p className="muted text-sm">المسودة: {d.draft_version||1} · المنشور: {d.published_version||0} · {d.is_published?'التصميم منشور':'غير منشور'}</p></div></Panel><div className="studio-side"><Panel title="معاينة فورية"><WalletCard design={d} brand={brandInfo} programs={programs}/><p className="muted text-sm mt-3">Apple Wallet يفرض أماكن محددة للنصوص، لكن الخلفية والشعار والشريط والألوان تُطبّق ضمن الحدود الرسمية.</p></Panel><Panel title="إصدار بطاقات العملاء" desc="بعد نشر التصميم، اختر عميلًا لإصدار الرابط"><div className="compact-list scroll-list">{customers.map((c:Row)=><div key={c.id}><div className="entity-main"><b>{c.name}</b><small>{caps.points?`${c.points} نقطة · `:''}{caps.stamps?`${c.stamps} ختم · `:''}{c.membership_code}</small></div><Btn busy={busy===`pass-${c.id}`} label="إصدار" icon={WalletCards} variant="secondary" onClick={()=>issuePass(c)}/></div>)}{!customers.length&&<Empty/>}</div></Panel></div></div>
}
...
function WalletCard({design,brand,programs=[]}:any){
  const hero=design.hero_url?`${API}/api/wallet/public/assets/${brand?.id}/hero`:''
  const logo=design.logo_url?`${API}/api/wallet/public/assets/${brand?.id}/logo`:''
  const background=design.background_image_url?`${API}/api/wallet/public/assets/${brand?.id}/background`:''
  const overlay=Math.max(0,Math.min(90,Number(design.overlay_opacity||0)))/100
  const style:any={backgroundColor:design.background_color,color:design.foreground_color}
  if(background)style.backgroundImage=`linear-gradient(rgba(0,0,0,${overlay}),rgba(0,0,0,${overlay})),url(${background})`
  return <div className={`wallet-preview layout-${design.layout_style||'classic'}`} style={style}>{hero&&<img className="wallet-hero-image" src={hero} alt=""/>}<div className="wallet-header"><div><small style={{color:design.label_color}}>MEMBER CARD</small><h3>{design.logo_text||brand?.name}</h3></div>{logo?<img className="wallet-logo-image" src={logo} alt="شعار البراند"/>:<div className="wallet-logo">{(brand?.name||'L')[0]}</div>}</div><p className="wallet-title">{design.card_title}</p>{design.fields.show_stamps&&programs.length>0?<div className="wallet-programs">{programs.slice(0,2).map((x:Row)=><div key={x.id}><span>{stampIcon(x.stamp_icon)}</span><div><small style={{color:design.label_color}}>{x.name}</small><b>4 / {x.required_stamps}</b></div></div>)}</div>:<div className="wallet-values">{design.fields.show_points&&<span><small style={{color:design.label_color}}>النقاط</small><b>1,250</b></span>}{design.fields.show_rewards&&<span><small style={{color:design.label_color}}>المكافآت</small><b>3</b></span>}{design.fields.show_stamps&&<span><small style={{color:design.label_color}}>الأختام</small><b>4 / 6</b></span>}</div>}<div className="wallet-footer"><span>{design.fields.show_tier?'ذهبي':''}</span><div className="qr-box">QR</div></div></div>
}
