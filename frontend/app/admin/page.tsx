...
const STAMP_ICON_CATEGORIES=[
  {key:'coffee',label:'قهوة ومشروبات',items:[['coffee','☕','كوب قهوة'],['espresso','☕','إسبريسو'],['bean','◉','حبة بن'],['cup','🥤','كوب سفري'],['cold_drink','🧋','مشروب بارد'],['tea','🍵','شاي'],['juice','🧃','عصير']]},
  {key:'dessert',label:'حلى ومخبوزات',items:[['cake','🍰','قطعة كيك'],['cookie','🍪','كوكيز'],['donut','🍩','دونات'],['croissant','🥐','كرواسون'],['cupcake','🧁','كب كيك'],['icecream','🍨','آيس كريم'],['chocolate','🍫','شوكولاتة']]},
  {key:'food',label:'طعام',items:[['breakfast','🍳','فطور'],['burger','🍔','برغر'],['pizza','🍕','بيتزا'],['sandwich','🥪','ساندويتش'],['salad','🥗','سلطة']]},
  {key:'general',label:'رموز عامة',items:[['star','★','نجمة'],['heart','♥','قلب'],['gift','🎁','هدية'],['crown','♛','تاج'],['sparkle','✦','لمعة'],['custom','●','صورة مخصصة']]},
] as const
const DEFAULT_STAMP_SETTINGS={display_mode:'icons_and_count',stamp_shape:'circle',empty_style:'outline',filled_style:'solid',icon_size:'medium',allow_multiple:true,max_per_action:5,daily_limit:null,carry_over:true,show_reward_title:true,show_on_wallet_front:true,allowed_branch_ids:[],starts_at:null,ends_at:null}
...
function CardTemplates({rows,programs,brand,brandInfo,busy,run,reload}:any){
  ...
  const templatePayload=(fd:FormData)=>({
    ...
    fields:{show_stamps:true,show_rewards:fd.get('show_rewards')==='on',show_points:false,show_tier:false,show_visits:false,show_member_name:fd.get('show_member_name')==='on',show_qr:fd.get('show_qr')==='on',show_reward_line:fd.get('show_reward_line')==='on',background_mode:fd.get('background_mode')||'solid',gradient_start:fd.get('gradient_start')||fd.get('background_color'),gradient_end:fd.get('gradient_end')||fd.get('background_color'),image_fit:fd.get('image_fit')||'cover',image_position:fd.get('image_position')||'center',stamp_display_mode:fd.get('stamp_display_mode')||'program',max_front_programs:Number(fd.get('max_front_programs')||2),card_corner_style:fd.get('card_corner_style')||'rounded'},
  ...
  async function removeTemplateAsset(x:Row,kind:string){if(!confirm('حذف الصورة والعودة للشكل الافتراضي؟'))return;await run(`card-asset-delete-${x.id}-${kind}`,()=>api(`/api/cards/templates/${x.id}/asset/${kind}`,{method:'DELETE'}),'تم حذف الصورة');await reload()}
  ...
  return <>
    <div className="content-grid mt-6">
      <Panel title="إنشاء بطاقة جديدة" desc="اختر برامج الأختام التي ستظهر داخل بطاقة Apple Wallet الواحدة">
        <form className="form-grid" onSubmit={create}>
          ...
          <CardDesignFields brandInfo={brandInfo}/>
    ...
    <Panel title="قوالب البطاقات" desc="بطاقة قهوة فقط، قهوة وحلى، فطور أو أي تركيبة تريدها" className="mt-5">
      {templates.length?<div className="template-grid">{templates.map((x:Row)=><article className={`template-card status-${x.status}`} key={x.id}>
        ...
        <div className="asset-grid four compact-assets"><AssetControl label="الشعار" hasAsset={!!x.logo_url} busy={busy===`card-asset-${x.id}-logo`} onUpload={(file:File)=>asset(x,'logo',file)} onRemove={()=>removeTemplateAsset(x,'logo')}/><AssetControl label="صورة علوية" hasAsset={!!x.hero_url} busy={busy===`card-asset-${x.id}-hero`} onUpload={(file:File)=>asset(x,'hero',file)} onRemove={()=>removeTemplateAsset(x,'hero')}/><AssetControl label="خلفية كاملة" hasAsset={!!x.background_image_url} busy={busy===`card-asset-${x.id}-background`} onUpload={(file:File)=>asset(x,'background',file)} onRemove={()=>removeTemplateAsset(x,'background')}/><AssetControl label="شريط Apple" hasAsset={!!x.strip_url} busy={busy===`card-asset-${x.id}-strip`} onUpload={(file:File)=>asset(x,'strip',file)} onRemove={()=>removeTemplateAsset(x,'strip')}/></div>
...
function CardDesignFields({brandInfo,design}:any){
  const fields={show_member_name:true,show_qr:true,show_reward_line:true,show_rewards:true,background_mode:'solid',gradient_start:design?.background_color||brandInfo?.primary_color||'#111827',gradient_end:design?.background_color||'#111827',image_fit:'cover',image_position:'center',stamp_display_mode:'program',max_front_programs:2,card_corner_style:'rounded',...(design?.fields||{})}
  return <>
    <div className="form-divider">تصميم البطاقة</div>
    <div className="design-preset-grid">
      {[['classic','كلاسيكي'],['visual','صورة كاملة'],['minimal','بسيط'],['luxury','فاخر'],['product','منتج'],['light','فاتح'],['dark','داكن']].map(([value,label])=><label key={value}><input type="radio" name="layout_style" value={value} defaultChecked={(design?.layout_style||'classic')===value}/><span>{label}</span></label>)}
    </div>
    <div className="two-col"><Field label="لون الخلفية" name="background_color" type="color" defaultValue={design?.background_color||brandInfo?.primary_color||'#111827'}/><Field label="لون النص" name="foreground_color" type="color" defaultValue={design?.foreground_color||'#FFFFFF'}/></div>
    <div className="two-col"><Field label="لون العناوين" name="label_color" type="color" defaultValue={design?.label_color||brandInfo?.accent_color||'#C6FF4A'}/><Field label="تعتيم الصورة %" name="overlay_opacity" type="number" defaultValue={design?.overlay_opacity??25} min="0" max="90"/></div>
    <div className="two-col"><Field label="اسم الشعار" name="logo_text" defaultValue={design?.logo_text||brandInfo?.name||'LOYALYN'}/><Field label="عنوان البطاقة" name="card_title" defaultValue={design?.card_title||'بطاقة الولاء'}/></div>
    <div className="three-col"><Field label="نوع الخلفية"><select className="input" name="background_mode" defaultValue={fields.background_mode}><option value="solid">لون ثابت</option><option value="gradient">تدرج لوني</option><option value="image">صورة كاملة</option><option value="image_gradient">صورة + تدرج</option></select></Field><Field label="بداية التدرج" name="gradient_start" type="color" defaultValue={fields.gradient_start}/><Field label="نهاية التدرج" name="gradient_end" type="color" defaultValue={fields.gradient_end}/></div>
    <div className="three-col"><Field label="ملاءمة الصورة"><select className="input" name="image_fit" defaultValue={fields.image_fit}><option value="cover">تغطية كاملة</option><option value="contain">إظهار كاملة</option><option value="fill">تمديد</option></select></Field><Field label="موضع الصورة"><select className="input" name="image_position" defaultValue={fields.image_position}><option value="center">وسط</option><option value="top">أعلى</option><option value="bottom">أسفل</option><option value="left">يسار</option><option value="right">يمين</option></select></Field><Field label="حواف البطاقة"><select className="input" name="card_corner_style" defaultValue={fields.card_corner_style}><option value="rounded">دائرية</option><option value="soft">ناعمة</option><option value="square">مربعة</option></select></Field></div>
    <div className="three-col"><Field label="عرض برامج الأختام"><select className="input" name="stamp_display_mode" defaultValue={fields.stamp_display_mode}><option value="program">حسب إعداد كل برنامج</option><option value="compact">مختصر</option><option value="icons">أيقونات</option><option value="count">أرقام فقط</option></select></Field><Field label="البرامج على الواجهة" name="max_front_programs" type="number" defaultValue={fields.max_front_programs||2} min="1" max="3"/><Field label="نوع الباركود"><select className="input" name="barcode_format" defaultValue={design?.barcode_format||'PKBarcodeFormatQR'}><option value="PKBarcodeFormatQR">QR</option><option value="PKBarcodeFormatPDF417">PDF417</option><option value="PKBarcodeFormatAztec">Aztec</option><option value="PKBarcodeFormatCode128">Code 128</option></select></Field></div>
    <div className="switch-grid"><Switch label="إظهار اسم العميل" name="show_member_name" defaultChecked={fields.show_member_name!==false}/><Switch label="إظهار QR" name="show_qr" defaultChecked={fields.show_qr!==false}/><Switch label="إظهار سطر المكافأة" name="show_reward_line" defaultChecked={fields.show_reward_line!==false}/><Switch label="إظهار المكافآت" name="show_rewards" defaultChecked={fields.show_rewards!==false}/></div>
  </>
}
...
function StampCards({rows,brand,brandInfo,busy,run,reload}:any){
  ...
  const payload=(fd:FormData)=>({
    ...
    settings:{
      display_mode:fd.get('display_mode')||'icons_and_count',stamp_shape:fd.get('stamp_shape')||'circle',
      empty_style:fd.get('empty_style')||'outline',filled_style:fd.get('filled_style')||'solid',
      icon_size:fd.get('icon_size')||'medium',allow_multiple:fd.get('allow_multiple')==='on',
      max_per_action:Number(fd.get('max_per_action')||1),daily_limit:fd.get('daily_limit')?Number(fd.get('daily_limit')):null,
      carry_over:fd.get('carry_over')==='on',show_reward_title:fd.get('show_reward_title')==='on',
      show_on_wallet_front:fd.get('show_on_wallet_front')==='on',allowed_branch_ids:fd.getAll('allowed_branch_ids').map(String),
  ...
  async function create(e:React.FormEvent<HTMLFormElement>){e.preventDefault();const form=e.currentTarget;await run('stamp-program-create',()=>api('/api/stamps/programs',{method:'POST',body:JSON.stringify(payload(new FormData(form)))}),'تم إنشاء برنامج الختم وإضافته للبطاقة الافتراضية');form.reset();await reload()}
  ...
  async function duplicate(program:Row){await run(`stamp-duplicate-${program.id}`,()=>api(`/api/stamps/programs/${program.id}/duplicate`,{method:'POST'}),'تم نسخ البرنامج مع الصور والإعدادات');await reload()}
  async function move(program:Row,direction:-1|1){
    const active=programs.filter(x=>!x.is_archived).sort((a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0));const index=active.findIndex(x=>x.id===program.id),target=index+direction
    if(index<0||target<0||target>=active.length)return
    const ordered=[...active];[ordered[index],ordered[target]]=[ordered[target],ordered[index]]
    await run(`stamp-reorder-${program.id}`,()=>api(`/api/stamps/programs/reorder?brand_id=${brand}`,{method:'POST',body:JSON.stringify({program_ids:ordered.map(x=>x.id)})}),'تم تغيير ترتيب البرامج');await reload()
  }
  ...
  async function removeAsset(program:Row,kind:string){if(!confirm('حذف هذه الصورة والعودة للشكل الافتراضي؟'))return;await run(`stamp-asset-delete-${program.id}-${kind}`,()=>api(`/api/stamps/programs/${program.id}/asset/${kind}`,{method:'DELETE'}),'تم حذف الصورة');await reload()}
  return <>
    <div className="content-grid mt-6">
      <Panel title="إنشاء برنامج ختم" desc="أنشئ قهوة أو حلى أو أي مسمى، ثم اجمع أكثر من برنامج داخل بطاقة Wallet واحدة.">
        <form className="form-grid" onSubmit={create}><StampProgramFields brandInfo={brandInfo}/><Btn busy={busy==='stamp-program-create'} label="إنشاء برنامج الختم" icon={Plus}/></form>
      </Panel>
      <Panel title="ترتيب العمل الصحيح" desc="البرنامج هو قسم داخل البطاقة، وليس بطاقة Apple مستقلة.">
        <div className="rules-box"><Rule icon={Stamp} title="1. أنشئ البرامج" text="قهوة، حلى، فطور أو أي مسمى تختاره."/><Rule icon={Layers3} title="2. اجمعها في بطاقة" text="من قسم البطاقات اختر قهوة وحلى داخل نفس القالب."/><Rule icon={WalletCards} title="3. بطاقة واحدة للعميل" text="الموظف يختار الختم المناسب بعد السكان."/></div>
        <div className="quick-card compact-quick"><Sparkles/><b>كل الصور قابلة للتغيير</b><span>خلفية القسم، الختم الفارغ، الختم الممتلئ، أو رمز جاهز من المكتبة.</span></div>
      </Panel>
    </div>
    <Panel title="مكتبة برامج الأختام" desc="عدّل، انسخ، أعد الترتيب، ارفع صورك أو استخدم الرموز الجاهزة." className="mt-5">
      {programs.length?<div className="stamp-program-grid">{programs.map((program:Row,index:number)=><article className={`stamp-program-card ${!program.is_active||program.is_archived?'disabled':''}`} key={program.id} style={{backgroundColor:program.background_color,color:'#fff',backgroundImage:program.card_image_url?`linear-gradient(rgba(0,0,0,.25),rgba(0,0,0,.5)),url(${API}/api/stamps/public/assets/${program.id}/card)`:undefined}}>
        ...
        <div className="stamp-program-settings-summary"><span>{displayModeName(program.settings?.display_mode)}</span><span>{shapeName(program.settings?.stamp_shape)}</span><span>حد العملية {program.settings?.max_per_action||1}</span></div>
        <div className="stamp-assets">
          <AssetControl label="صورة القسم" hasAsset={!!program.card_image_url} busy={busy===`stamp-asset-${program.id}-card`} onUpload={(file:File)=>asset(program,'card',file)} onRemove={()=>removeAsset(program,'card')}/>
          <AssetControl label="الختم الفارغ" hasAsset={!!program.empty_stamp_image_url} busy={busy===`stamp-asset-${program.id}-empty_stamp`} onUpload={(file:File)=>asset(program,'empty_stamp',file)} onRemove={()=>removeAsset(program,'empty_stamp')}/>
          <AssetControl label="الختم الممتلئ" hasAsset={!!program.filled_stamp_image_url} busy={busy===`stamp-asset-${program.id}-filled_stamp`} onUpload={(file:File)=>asset(program,'filled_stamp',file)} onRemove={()=>removeAsset(program,'filled_stamp')}/>
        </div>
        <div className="button-row wrap-actions"><Btn label="تعديل كامل" icon={Pencil} variant="secondary" onClick={()=>setEditing(program)}/><Btn label="معاينة" icon={Eye} variant="secondary" onClick={()=>setPreview(program)}/><Btn busy={busy===`stamp-duplicate-${program.id}`} label="نسخ" icon={Copy} variant="secondary" onClick={()=>duplicate(program)}/><Btn label="↑" variant="secondary" disabled={index===0} onClick={()=>move(program,-1)}/><Btn label="↓" variant="secondary" disabled={index===programs.length-1} onClick={()=>move(program,1)}/>{program.is_archived?<Btn busy={busy===`stamp-restore-${program.id}`} label="استعادة" icon={RotateCcw} variant="secondary" onClick={()=>action(program,'restore')}/>:<Btn busy={busy===`stamp-archive-${program.id}`} label="أرشفة" icon={Archive} variant="secondary" onClick={()=>action(program,'archive')}/>}<Btn busy={busy===`stamp-delete-${program.id}`} label="حذف" icon={Trash2} variant="danger" onClick={()=>remove(program)}/></div>
      </article>)}</div>:<Empty text="أنشئ أول برنامج ختم"/>}
    </Panel>
    {editing&&<Modal title={`تعديل ${editing.name}`} desc="يمكنك تغيير الاسم والرمز والصور والقواعد بدون حذف سجل العملاء" onClose={()=>setEditing(null)}><form className="form-grid" onSubmit={update}><StampProgramFields brandInfo={brandInfo} program={editing}/><Switch label="البرنامج فعال" name="is_active" defaultChecked={editing.is_active}/><div className="button-row"><Btn busy={busy===`stamp-edit-${editing.id}`} label="حفظ التعديلات"/><Btn type="button" label="إلغاء" variant="secondary" onClick={()=>setEditing(null)}/></div></form></Modal>}
    {preview&&<Modal title={`معاينة ${preview.name}`} desc="معاينة شكل البرنامج داخل البطاقة وشاشة الموظف" onClose={()=>setPreview(null)}><article className="scan-stamp-card preview-stamp-program" style={{backgroundColor:preview.background_color,backgroundImage:preview.card_image_url?`linear-gradient(rgba(0,0,0,.2),rgba(0,0,0,.45)),url(${API}/api/stamps/public/assets/${preview.id}/card)`:undefined}}><div className="stamp-program-head"><span>{stampIcon(preview.stamp_icon)}</span><div><b>{preview.name}</b><small>{preview.reward_title}</small></div></div><StampDots count={Math.min(4,preview.required_stamps)} total={preview.required_stamps} program={preview}/><div className="scan-card-meta"><span><b>{Math.min(4,preview.required_stamps)}</b>/{preview.required_stamps} ختم</span><span>{displayModeName(preview.settings?.display_mode)}</span></div></article></Modal>}
  </>
}

function StampProgramFields({brandInfo,program}:any){
  const settings={...DEFAULT_STAMP_SETTINGS,...(program?.settings||{})}
  const [icon,setIcon]=useState(program?.stamp_icon||'coffee')
  useEffect(()=>setIcon(program?.stamp_icon||'coffee'),[program?.id])
  return <>
    <div className="two-col"><Field label="اسم البرنامج" name="name" defaultValue={program?.name||''} placeholder="قهوة"/><Field label="الرابط الداخلي" name="slug" defaultValue={program?.slug||''} placeholder="coffee" pattern="[a-z0-9-]+"/></div>
    <Field label="الوصف" name="description" defaultValue={program?.description||''} required={false}/>
    <div className="two-col"><Field label="عدد الأختام المطلوبة" name="required_stamps" type="number" defaultValue={program?.required_stamps||6} min="1" max="100"/><Field label="اسم المكافأة" name="reward_title" defaultValue={program?.reward_title||'مشروب مجاني'}/></div>
    <div className="two-col"><Field label="نوع المكافأة"><select className="input" name="reward_type" defaultValue={program?.reward_type||'free_item'}><option value="free_item">منتج مجاني</option><option value="discount">خصم</option><option value="custom">مخصص</option></select></Field><Field label="الترتيب" name="sort_order" type="number" defaultValue={program?.sort_order||0}/></div>
    <IconPicker value={icon} onChange={setIcon}/><input type="hidden" name="stamp_icon" value={icon}/>
    <div className="form-divider">شكل الأختام</div>
    <div className="two-col"><Field label="طريقة العرض"><select className="input" name="display_mode" defaultValue={settings.display_mode}><option value="icons_and_count">الأيقونات والعدد</option><option value="icons_only">الأيقونات فقط</option><option value="count_only">العدد فقط</option><option value="progress">شريط تقدم</option></select></Field><Field label="شكل الختم"><select className="input" name="stamp_shape" defaultValue={settings.stamp_shape}><option value="circle">دائري</option><option value="rounded">مربع بحواف</option><option value="square">مربع</option><option value="diamond">ماسي</option><option value="none">بدون إطار</option></select></Field></div>
    <div className="three-col"><Field label="الختم الفارغ"><select className="input" name="empty_style" defaultValue={settings.empty_style}><option value="outline">حد خارجي</option><option value="faded">باهت</option><option value="dot">نقطة</option><option value="hidden">مخفي</option></select></Field><Field label="الختم الممتلئ"><select className="input" name="filled_style" defaultValue={settings.filled_style}><option value="solid">ملون</option><option value="glow">متوهج</option><option value="outline">حد ملون</option></select></Field><Field label="حجم الرمز"><select className="input" name="icon_size" defaultValue={settings.icon_size}><option value="small">صغير</option><option value="medium">متوسط</option><option value="large">كبير</option></select></Field></div>
    <div className="two-col"><Field label="لون خلفية القسم" name="background_color" type="color" defaultValue={program?.background_color||brandInfo?.primary_color||'#111827'}/><Field label="لون الختم" name="accent_color" type="color" defaultValue={program?.accent_color||brandInfo?.accent_color||'#C6FF4A'}/></div>
    <div className="form-divider">قواعد الإضافة</div>
    <div className="three-col"><Field label="أقصى أختام في العملية" name="max_per_action" type="number" defaultValue={settings.max_per_action||1} min="1" max="20"/><Field label="الحد اليومي للعميل" name="daily_limit" type="number" defaultValue={settings.daily_limit??''} min="1" max="100" required={false}/><Field label="يبدأ في" name="starts_at" type="datetime-local" defaultValue={dateTimeInput(settings.starts_at)} required={false}/></div>
    <Field label="ينتهي في" name="ends_at" type="datetime-local" defaultValue={dateTimeInput(settings.ends_at)} required={false}/>
    <div className="switch-grid"><Switch label="السماح بأكثر من ختم في العملية" name="allow_multiple" defaultChecked={settings.allow_multiple!==false}/><Switch label="الاحتفاظ بالأختام الزائدة" name="carry_over" defaultChecked={settings.carry_over!==false}/><Switch label="إظهار اسم المكافأة" name="show_reward_title" defaultChecked={settings.show_reward_title!==false}/><Switch label="إظهار البرنامج على واجهة Wallet" name="show_on_wallet_front" defaultChecked={settings.show_on_wallet_front!==false}/><Switch label="البرنامج الافتراضي" name="is_default" defaultChecked={!!program?.is_default}/></div>
  </>
}

function IconPicker({value,onChange}:any){
  const [category,setCategory]=useState('coffee')
  const current=STAMP_ICON_CATEGORIES.find(x=>x.key===category)||STAMP_ICON_CATEGORIES[0]
  return <div className="field"><span>مكتبة رموز الختم</span><div className="icon-category-tabs">{STAMP_ICON_CATEGORIES.map(cat=><button type="button" key={cat.key} className={category===cat.key?'active':''} onClick={()=>setCategory(cat.key)}>{cat.label}</button>)}</div><div className="stamp-icon-grid">{current.items.map(([key,symbol,label])=><button type="button" key={key} className={value===key?'selected':''} onClick={()=>onChange(key)}><b>{symbol}</b><span>{label}</span></button>)}</div><small className="field-help">اختر «صورة مخصصة» ثم ارفع صورة للختم الفارغ والممتلئ بعد إنشاء البرنامج.</small></div>
}

function AssetControl({label,hasAsset,busy,onUpload,onRemove}:any){return <div className="asset-control"><label className="asset-upload"><Upload size={17}/><span>{busy?'جاري الرفع...':hasAsset?`استبدال ${label}`:`رفع ${label}`}</span><input type="file" accept="image/png,image/jpeg,image/webp" disabled={busy} onChange={e=>{const file=e.target.files?.[0];if(file)onUpload(file);e.currentTarget.value=''}}/></label>{hasAsset&&<button type="button" className="asset-remove" onClick={onRemove}><Trash2 size={15}/> حذف</button>}</div>}
...
function FastScan({brand,branches,busy,run,permissions}:any){
  ...
  async function add(program:Row,quantity=1){if(!result)return;const valueData=await run(`scan-add-${program.id}-${quantity}`,()=>api(`/api/stamps/customers/${result.customer.id}/programs/${program.id}/add`,{method:'POST',body:JSON.stringify({branch_id:branch||null,quantity,idempotency_key:crypto.randomUUID()})}),`تمت إضافة ${quantity} ختم إلى ${program.name}`);if(valueData){setResult({...result,card_template:valueData.card_template||result.card_template,cards:safeArray(valueData.cards)});await loadHistory(result.customer.id)}}
...
function StampDots({count,total,program}:any){
  const settings={...DEFAULT_STAMP_SETTINGS,...(program?.settings||{})},display=settings.display_mode||'icons_and_count'
  const shown=Math.min(Number(total)||0,12),shape=`shape-${settings.stamp_shape||'circle'}`,size=`size-${settings.icon_size||'medium'}`
  if(display==='count_only')return <div className="stamp-count-only"><b>{Number(count||0)}</b><span>/ {Number(total||0)} ختم</span></div>
  if(display==='progress')return <div className="stamp-progress"><div><i style={{width:`${Math.min(100,(Number(count||0)/Math.max(1,Number(total||1)))*100)}%`,backgroundColor:program.accent_color}}/></div><span>{Number(count||0)} / {Number(total||0)}</span></div>
  return <div className={`stamp-dots ${shape} ${size}`}>{Array.from({length:shown}).map((_,i)=>{const filled=i<Number(count||0);const asset=filled?program.filled_stamp_image_url:program.empty_stamp_image_url;const hidden=!filled&&settings.empty_style==='hidden';return <span key={i} className={`${filled?'filled':''} empty-${settings.empty_style||'outline'} filled-${settings.filled_style||'solid'} ${hidden?'hidden-stamp':''}`} style={{borderColor:program.accent_color,backgroundColor:filled&&!asset&&settings.filled_style!=='outline'?program.accent_color:undefined,color:filled&&settings.filled_style!=='outline'?'#0b0d11':program.accent_color}}>{asset?<img src={`${API}/api/stamps/public/assets/${program.id}/${filled?'filled_stamp':'empty_stamp'}`} alt=""/>:filled?stampIcon(program.stamp_icon):settings.empty_style==='dot'?'·':''}</span>})}{Number(total)>12&&<em>+{Number(total)-12}</em>}{display==='icons_and_count'&&<strong>{Number(count||0)} / {Number(total||0)}</strong>}</div>
}
const STAMP_SYMBOLS=Object.fromEntries(STAMP_ICON_CATEGORIES.flatMap(category=>category.items.map(([key,symbol])=>[key,symbol]))) as Row
const stampIcon=(value:string)=>STAMP_SYMBOLS[value]||'●'
const displayModeName=(value:string)=>({icons_and_count:'أيقونات + عدد',icons_only:'أيقونات فقط',count_only:'العدد فقط',progress:'شريط تقدم'} as Row)[value]||'أيقونات + عدد'
const shapeName=(value:string)=>({circle:'دائري',rounded:'مربع بحواف',square:'مربع',diamond:'ماسي',none:'بدون إطار'} as Row)[value]||'دائري'
...
function WalletCard({design,brand,programs=[]}:any){
  const fields={show_member_name:true,show_qr:true,show_reward_line:true,max_front_programs:2,background_mode:'solid',gradient_start:design.background_color||'#111827',gradient_end:design.background_color||'#111827',image_fit:'cover',image_position:'center',card_corner_style:'rounded',...emptyDesign.fields,...(design.fields||{})}
  ...
  const hero=resolveAsset(design.hero_url,'hero'),logo=resolveAsset(design.logo_url,'logo'),background=resolveAsset(design.background_image_url,'background')
  const overlay=Math.max(0,Math.min(90,Number(design.overlay_opacity||0)))/100
  const style:any={backgroundColor:design.background_color||'#111827',color:design.foreground_color||'#FFFFFF',backgroundPosition:fields.image_position||'center',backgroundSize:fields.image_fit||'cover'}
  const gradient=`linear-gradient(135deg,${fields.gradient_start||design.background_color||'#111827'},${fields.gradient_end||design.background_color||'#111827'})`
  ...
  const maxPrograms=Math.max(1,Math.min(3,Number(fields.max_front_programs||2))),rows=safeArray<Row>(programs).filter(x=>x.settings?.show_on_wallet_front!==false).slice(0,maxPrograms),sample=[4,2,1]
  return <div className={`wallet-preview loyalty-card-preview layout-${design.layout_style||'classic'} corners-${fields.card_corner_style||'rounded'}`} style={style}>
    {hero&&<img className="wallet-hero-image" style={{objectFit:fields.image_fit||'cover',objectPosition:fields.image_position||'center'}} src={hero} alt=""/>}
    ...
    {fields.show_member_name!==false&&<div className="wallet-member-line"><div><small style={{color:design.label_color||'#C6FF4A'}}>{design.card_title||'بطاقة الولاء'}</small><b>أحمد</b></div><span className="member-icon">◯</span></div>}
    {fields.show_stamps&&rows.length>0?<div className="wallet-program-list">{rows.map((x:Row,index:number)=>{const current=Math.min(sample[index]||1,Math.max(1,Number(x.required_stamps||6)-1)),total=Math.max(1,Number(x.required_stamps||6)),mode=fields.stamp_display_mode==='program'?x.settings?.display_mode:fields.stamp_display_mode==='count'?'count_only':fields.stamp_display_mode==='icons'?'icons_only':x.settings?.display_mode;return <div className="wallet-program-row" key={x.id||x.name}><div className="wallet-program-title"><span>{stampIcon(x.stamp_icon)}</span><div><small style={{color:design.label_color||'#C6FF4A'}}>{x.name}</small><b>{current} / {total}</b></div></div>{mode!=='count_only'&&<div className="wallet-stamp-dots" aria-label={`${current} من ${total}`}>{Array.from({length:Math.min(total,8)}).map((_,dot)=><i key={dot} className={dot<current?'filled':''}>{stampIcon(x.stamp_icon)}</i>)}</div>}</div>})}</div>:<div className="wallet-values">{fields.show_points&&<span><small style={{color:design.label_color}}>النقاط</small><b>1,250</b></span>}{fields.show_rewards&&<span><small style={{color:design.label_color}}>المكافآت</small><b>3</b></span>}{fields.show_stamps&&<span><small style={{color:design.label_color}}>الأختام</small><b>4 / 6</b></span>}</div>}
    {fields.show_reward_line!==false&&<div className="wallet-reward-line"><Gift size={17}/><span>المكافأة القادمة: <b>{rows[0]?.reward_title||'مكافأة مجانية'}</b></span></div>}
    <div className="wallet-footer"><span>{fields.show_tier?'ذهبي':design.name||''}</span>{fields.show_qr!==false&&<div className="qr-box">QR</div>}</div>
  </div>
}
